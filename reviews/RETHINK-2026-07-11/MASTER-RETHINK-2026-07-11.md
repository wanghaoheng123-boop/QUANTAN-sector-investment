...
6. **D1 SHIPPED** (owner delegated: "make your own logical decision") — the benchmark's PRIMARY gate is now **edge-over-base-rate ≥ +1.81pp** (frozen +2.31pp − the established 50 bps tolerance); the raw net-WR floor (53.29) demoted to a secondary regression guard; a WARN prints while the non-overlap Wilson lower bound sits below the base rate (expected until significance). Gate verified passing; label numbers byte-identical. See invariants-baseline.md §1b amendment.
7. **D2/D4 SHIPPED** (same delegation) — both engines' default exit policy is now **label-matched time-only exits** (20 bars, T+1 open): ATR/trailing/break-even stops, panic exit, profit-take, and the falling-knife SELL exit retired; DD circuit breaker retained; legacy policy available via `exit: DEFAULT_EXIT_CONFIG`. Measured before → after on the fixtures: **portfolio engine +6.90% → +8.52% total return** (maxDD 9.88%), net WR 54.17% honest; **per-instrument WR 24.0% → 54.28%** (page copy finally true), per-instrument eq-weight return 2.63% → 1.23% (flat-0.15 sizing truncates the old accidental trend rides — per-dollar efficiency unchanged; sizing is D6, deliberately not bundled). Label WR verified byte-identical. Behavior pinned in tests (exitRules + portfolio exit-reason profile). See invariants-baseline.md §1d.
